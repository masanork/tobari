...
1.  **技術的成立性**: 項目単位の選択的開示（Selective Disclosure）を実装しても、閲覧・検証機能を内包して**200KB程度の現実的なファイルサイズ**に収まることを実証。
2.  **高度なセキュリティ（Binding）**: オフライン署名検証に加え、**Holder Binding**や**Device Binding**により、真正性担保と不正利用防止（所持者確認）を両立可能であることを確認。
3.  **氏名等の漢字の再現性**: **約7万文字の行政事務標準文字**を、環境非依存で表示可能であることを確認。
...
| レイヤー | 技術規格 | 役割 | 検証結果 |
| :--- | :--- | :--- | :--- |
| **UX/表示層** | **HTML5 + CSS + WOFF2** | 人間用表示 | 行政事務標準文字（約7万文字）を含むサブセットフォント埋め込みにより、**外字の完全再現に成功**。 |
...
#### (4) WebAssemblyによる高速な署名検証とJPKI通信
...
6.  **視覚的検証モデルとUXの在り方**
    ...
    *   **紙様式の模倣 vs デジタル最適化**: 従来の「紙の住民票」のレイアウトを画面上で再現すること（Fixed Layout）は、既存業務との親和性が高い一方で、例えばスマートフォンの小さな画面では視認性が著しく低下する。
    
    *   **意図的なデザインの差別化**: 電子交付されたファイルを「印刷して使用するもの」と誤認したり、紙の偽造（コピー）と混同されたりするリスクを防ぐため、**あえて紙とは全く異なるレスポンシブなデザインを採用することも考えられるのではないか**。本検証の「自己完結型ビューアー」はHTML/CSSベースであるため、閲覧デバイスに応じた最適な表示が可能であり、紙の制約に縛られず、操作性やアクセシビリティーに配慮したデザインを採用することも可能である。
