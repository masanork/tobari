...
subtitle: 中間報告書における指摘事項（真正性・転々流通防止等）の実装例
date: 2026-01-14
layout: article
author: 楠 正憲
embedFonts: true
...
**中間報告書における指摘事項（真正性・転々流通防止等）の実装例**

## 1. はじめに（検証の目的と位置付け）

本報告書は、検討会中間報告書等において提示された、「住民票の写し」等の電子交付に求められる高度な要件（真正性の担保、転々流通の防止、氏名等の漢字の正確な表示等）を、既存技術の組み合わせによりどこまで実現可能かを検証した技術報告である。

特に、先行して行われた検討会中間報告書等において指摘された **「転々流通（不正な使い回し）の防止」「真正性の担保」「氏名等の漢字の正確な表示」** といった課題に対し、Web標準技術（HTML/CSS）と最新の国際デジタルID標準（ISO 18013-5 mDoc / IETF SD-JWT）を融合させることで、いかに解決可能かを実証・検証した結果を示すものである。
...
* **機械可読性の欠如**: データとして再利用（入力自動化等）ができず、OCR等の処理が必要となる。特に氏名や住所に含まれる異体字・氏名等のを正確に認識することは技術的に困難である。
...
* **機械可読性の欠如**: PDFは表示の再現性を重視したフォーマットであり、構造化データとしての抽出・活用が困難である。特に氏名等の漢字が含まれる場合、文字コードとしての情報が失われていることが多く、データ連携の障害となる。
...
* **氏名等の漢字表示の壁**: 現在の標準的なVC仕様には、氏名・住所に含まれる本邦固有の「氏名等の漢字（行政事務標準文字）」を、受領者の環境に依存せず正確に表示・検証するための仕組みが含まれていない。
...
また、本検証を通じて「Webベースで動作する住民票VC」の具体的な技術仕様を早期に示すことは、単なる一時しのぎではない重要な意義を持つ。日本固有の要件（氏名等の漢字の正確な表示や厳格なレイアウト再現等）を実装可能な形で世界に示すことで、現在進行中の国際標準化プロセス（ISO/IETF/W3C等）に対して、日本からの要件をフィードバックし、標準仕様そのものに影響を与えていく（我々の要件を反映させる）ための布石ともなるものである。
...
| レイヤー | 技術規格 | 役割 | 検証結果 |
| :--- | :--- | :--- | :--- |
| **UX/表示層** | **HTML5 + CSS + WOFF2** | 人間用表示 | **IVS（異体字セレクタ）** に対応した書記素（Grapheme）単位のサブセット抽出により、**氏名等の漢字を含む行政事務標準文字の完全再現** に成功。 |
...
#### 3.2.1. 生成物サイズ（Core Build）

| 生成モード | .cose | .html |
| :--- | ---: | ---: |
| core（標準構成） | 4,096 bytes（gzip: 1,825） | 275,000 bytes（gzip: 138,192） |

HTMLファイルサイズは約275KB（Gzip圧縮時約138KB）であり、一般的な画像のファイルサイズと同等かそれ以下である。これにより、モバイル回線等でもストレスなくダウンロード・検証が可能である。
※耐量子暗号（PQC）適用時のサイズ影響については、別紙「証憑等の電子交付にかかる耐量子暗号の適用可能性について」を参照のこと。

#### 3.2.2. 暗号・検証の処理時間（参考値）
...
本PoCでは、将来的な量子コンピュータによる暗号解読リスク（CRQC）に備え、NIST FIPS 204 (ML-DSA) 等の耐量子暗号をサポートするアーキテクチャを採用している。
ビューアー自体が検証ロジックを内包しているため、将来的に使用する暗号アルゴリズムを変更する場合でも、**新しいアルゴリズムに対応した検証コードを含む新しいビューアーを配布するだけ** で、受信者（Verifier）側のシステム改修を待つことなく迅速な移行が可能である。
...
本技術検証により、電子交付における主要な技術課題（真正性・転々流通防止・氏名等の漢字表示）は解決可能であることが示された。一方で、実際の制度運用および社会実装に向けては、技術選定と並行して以下の点について整理が必要であると考えられる。
...
2. **選択的開示（Selective Disclosure）の適用範囲とデータサイズのトレードオフ**
    * **データサイズの内訳**: 本方式の成果物（HTML）は約200KBであるが、その大半は「ビューアー機能（検証ロジック・フォント・UI）」が占めており、証明データそのもののサイズは極めて小さい。
        * **ビューアー部**: 約280KB（mDoc/SD-JWT検証機能、氏名等の漢字フォント含む）※圧縮時
...
本技術検証により、HTML/CSSと最新の暗号技術規格（mDoc/SD-JWT）を組み合わせることで、中間報告書等で指摘された「真正性の担保」「転々流通防止」「氏名等の漢字表示」といった課題を解決し得る電子交付モデルが、技術的には実装可能であることが確認された。

特に、特定ベンダーのプロプライエタリな製品に依存せず、オープンなWeb標準技術のみで構成できることは、持続可能な公的インフラを設計する上で重要な示唆を与えるものである。今後は本検証で得られた技術的知見を基礎として、法令上の課題や実現に向けたより具体的な検討が進められることが期待される。

---

## 付録: データサイズ計測詳細報告

本検証において、選択的開示の粒度がデータサイズに与える影響を定量的に評価するため、実際の運用を想定した日本語データ（氏名等の漢字・IVS含む）を用いて実測を行った。
...
* **結論**: データの日本語化・IVS利用により全体サイズは微増したが、個人のプライバシー権を最大限尊重するため、**可能な限り細かい粒度（シナリオA）での選択的開示を実装することが考えられる**。フォーマット選定においては、サイズ効率を重視するならmDoc、Web親和性を重視するならSD-JWTとなるが、どちらも許容範囲内である。
