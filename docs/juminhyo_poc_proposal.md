...
PQC対応の有無およびアルゴリズム別のWASMサイズ差を測定した（以下は wasm-pack 出力の `.wasm` バイナリサイズ）。
...
耐量子署名（ML-DSA）の追加により約100KB、耐量子鍵交換（ML-KEM）の追加によりさらに約50KBのサイズ増加となる。フル構成においても、Gzip圧縮後の転送量増加は40KB〜50KB程度に抑制されることを確認した。
...
### 3.4. 新たな知見：AIとハードウェアの架け橋として

一連のPoC実装を通じて、Web標準技術とネイティブ機能を高度に融合させることで、以下の新たな知見が得られた。

#### (1) AI時代に向けた「機械可読性」の再定義（AI-Ready Identity）

これまでの「機械可読（Machine-Readable）」は、構造化されたデータ（CBOR/ASN.1等）をコンピュータ・プログラムがパースできることを指していた。
本PoCでは、複雑な mdoc 構造を LLM（大規模言語モデル）が理解・推論しやすい平易な JSON 形式へと再帰的に翻訳する機能を実装した。これにより、デジタル証明書は「プログラム用のデータ」から **「AIエージェントが活用可能な知識」** へと昇華された。
ユーザーの代理人（Agent）であるAIが証明書の内容を正確に解釈し、複雑な申請書の自動代筆や、資格要件の事前チェックを安全に行う「自己主権型エージェンシー（SSA）」の実現に向けた大きな一歩となった。

#### (2) 「ネイティブ・セキュリティ・サイドカー」パターンの有効性

ブラウザにおけるハードウェア制御（NFC/PCSC）や OS 標準のセキュリティ機能（Secure Enclave/TPM）へのアクセス制約を、軽量なネイティブ・バイナリ（Swift/Rust）を「サイドカー」として提供するパターンで解決した。
このアプローチにより、以下の2点を高い次元で両立できることを実証した。
*   **Webの利便性**: ユーザーインターフェースや申請フローは使い慣れたブラウザで提供。
*   **ハードウェアの堅牢性**: 暗号鍵の生成や署名、ICカードリーダーとの複雑な低レイヤー通信は、OSに最適化されたネイティブコードが担当。
特に、デバイスごとに異なるカードリーダーの挙動差異（タイムアウト制御や特殊なプロトコル対応）を、抽象化された JSON-RPC インターフェース（Unified Interface）で隠蔽し、Web層に一貫した体験を提供できる点は、実社会への普及において極めて重要である。

#### (3) 検証ロジックの配布による「信頼のポータビリティ」

検証コードを WebAssembly (WASM) として HTML 自体に内包しつつ、同時に CLI 検証ツールとしても提供するモデルの有効性を確認した。
特定の SDK やライブラリのインストールを強制することなく「ドキュメント自体が自分自身の検証方法を知っている」状態にすることで、受領者（Verifier）側の導入コストを劇的に下げられる。また、CLI ツールの提供は、既存のエンタープライズシステムや行政のバッチ処理系との親和性を確保し、技術仕様の透明性を担保する上で不可欠である。
...
Viewerに内包される暗号ライブラリ（Rust製WebAssembly）について、各PQCアルゴリズムの実装追加によるバイナリサイズの変遷を詳細に評価した。

| 構成 | 主要機能 | ファイルサイズ | 増分 |
| :--- | :--- | :--- | :--- |
| **Classic Only** | ECDSA(P-256), ECDH, AES-GCM | **141 KB** | - |
| **Classic + ML-DSA** | 上記 + ML-DSA-65 (Dilithium) | **242 KB** | **+101 KB** |
| **Full PQC (DSA+KEM)** | 上記 + ML-KEM-768 (Kyber) | **293 KB** | **+152 KB** |

#### 考察
耐量子署名（ML-DSA）の実装追加により約100KB、耐量子鍵交換（ML-KEM）の追加により約50KBの増加が確認された。
これは格子暗号に必要な行列演算ロジックや巨大な定数テーブルによるものである。特にML-DSAは署名検証に必要な公開鍵サイズや計算テーブルが大きいため、ML-KEMと比較してサイズ増加が顕著となる傾向がある。
現在のブロードバンド環境およびスマートフォン等のリソースにおいては、数十KBから百数十KB程度の増加はUXを損なうレベルではなく、暗号学的敏捷性（Crypto Agility）を確保するメリットが上回ると判断される。
