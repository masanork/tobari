...
async fn perform_sign(state: State<'_, AppState>, app: AppHandle) -> Result<(), String> {
    ...
    // Convert Challenge
    let challenge_bytes = URL_SAFE_NO_PAD.decode(&request.challenge)
    ...
    let result = tokio::task::spawn_blocking(move || {
        ...
        let allowed_creds = request.allow_credentials.as_ref().map(|creds| {
            creds.iter().map(|c| {
                authenticator::state::Descriptor {
        ...
        let flags = authenticator::state::GetAssertionFlags {
            user_presence: true,
            user_verification: match request.user_verification.as_deref() {
                Some("required") => true,
                _ => false, // default preferred usually implies true for platform
            },
            ..Default::default()
        };
        ...
        // Using `get_assertion` from `authenticator`
        match manager.get_assertion(
            &rp_id, 
            &challenge_bytes, 
            &allowed_creds, 
            flags, 
            // The callback for status updates
            |_status| {
...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![get_pending_request, perform_sign, reject])
