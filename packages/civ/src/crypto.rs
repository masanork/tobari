...
pub fn verify_x509_signature(cert_der: &[u8], message: &[u8], signature_bytes: &[u8]) -> Result<()> {
    ...
    if check_oid(oid, &[1, 2, 840, 113549, 1, 1, 1]) {
        ...
        pub_key.verify(Pkcs1v15Sign::new::<Sha256>(), &hashed, signature_bytes)
        ...
        let p256_oid = x509_parser::der_parser::oid::Oid::from(&[1, 2, 840, 10045, 3, 1, 7]).unwrap();
        
        // Simple check on parameters (assuming it's just the OID)
        let params_oid = params.as_oid()
