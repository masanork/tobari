...
use der_parser::ber::{parse_ber_octetstring, parse_ber_sequence, BerObjectContent};
...
impl PassportVerifier {
    ...
    pub fn parse_sod(&self, sod_data: &[u8]) -> Result<SecurityObjectDocument> {
        ...
        let ci_seq = content_info
            .content
            .as_sequence()
            .map_err(|_| CivError::InvalidData("ContentInfo not a sequence".to_string()))?;

        if ci_seq.len() < 2 {
            return Err(CivError::InvalidData("Invalid ContentInfo".to_string()));
        }
        ...
        let signed_data_bytes = match &ci_seq[1].content {
            ...
            BerObjectContent::Tagged(_, tag, inner) if tag.0 == 0 => {
                inner.as_slice().unwrap_or(&[])
            }
            _ => {
                return Err(CivError::InvalidData(
                    "ContentInfo content not tagged [0]".to_string(),
                ))
            }
        };

        let (_, signed_data_obj) = parse_ber_sequence(signed_data_bytes).map_err(|_| {
            CivError::InvalidData("Failed to parse SignedData sequence".to_string())
        })?;

        let signed_data_seq = signed_data_obj
            .content
            .as_sequence()
            .map_err(|_| CivError::InvalidData("SignedData not a sequence".to_string()))?;

        if signed_data_seq.len() < 3 {
            return Err(CivError::InvalidData("SignedData too short".to_string()));
        }

        let encap_content_info = &signed_data_seq[2];
        let encap_seq = encap_content_info
            .content
            .as_sequence()
        ...
        if encap_seq.len() < 2 {
            return Err(CivError::InvalidData(
                "EncapContentInfo missing content".to_string(),
            ));
        }
        ...
        let e_content_octet = e_content_obj
            .content
            .as_slice()
        ...
        for item in signed_data_seq.iter().skip(3) {
            match &item.content {
                BerObjectContent::Tagged(class, tag, inner)
                    if class == &Class::ContextSpecific && tag.0 == 0 =>
                {
                ...
                {
                    let (_, certs_obj) =
                        parse_ber_sequence(any.data).unwrap_or((&[], content_info.clone()));
    ...
    fn parse_lds_security_object(&self, data: &[u8]) -> Result<LdsSecurityObject> {
        ...
        if lds_seq.len() < 3 {
            return Err(CivError::InvalidData("LDS sequence too short".to_string()));
        }

        let hash_algo_oid = match &lds_seq[1].content {
            BerObjectContent::Sequence(s) => match &s[0].content {
                BerObjectContent::OID(oid) => oid.to_string(),
                _ => {
                    return Err(CivError::InvalidData(
                        "Invalid hash algorithm OID".to_string(),
                    ))
                }
            },
            _ => {
                return Err(CivError::InvalidData(
                    "Invalid AlgorithmIdentifier".to_string(),
                ))
            }
        };
        ...
        let hash_list_seq = match &lds_seq[2].content {
            ...
            _ => {
                return Err(CivError::InvalidData(
                    "DataGroupHashValues not a sequence".to_string(),
                ))
            }
        };
        ...
        for item in hash_list_seq {
            ...
            let dg_num = item_seq[0]
                .content
                .as_u32()
                .map_err(|_| CivError::InvalidData("Invalid DG num".to_string()))?
                as u8;
            let dg_hash = item_seq[1]
                .content
                .as_slice()
                .map_err(|_| CivError::InvalidData("Invalid hash bytes".to_string()))?
                .to_vec();
    ...
    pub fn verify_data_group(
        ...
        let actual_hash = match sod.lds_object.hash_algorithm.as_str() {
            ...
            _ => {
                return Err(CivError::CryptoError(format!(
                    "Unsupported hash algorithm: {}",
                    sod.lds_object.hash_algorithm
                )))
            }
        };
        ...
        if &actual_hash != expected_hash {
            return Err(CivError::InvalidData(format!(
                "Hash mismatch for DG {}: expected {}, got {}",
                dg_number,
    ...
    pub fn verify_passive_authentication(&self, sod: &SecurityObjectDocument) -> Result<()> {
        ...
        if let Some(attrs_bytes) = &sod.signed_attributes {
            ...
            let actual_hash = match sod.lds_object.hash_algorithm.as_str() {
                ...
                _ => {
                    return Err(CivError::CryptoError(
            ...
            let (_, attrs_obj) = parse_ber_sequence(attrs_bytes).map_err(|_| {
                CivError::InvalidData("Failed to parse signed attributes".to_string())
            })?;
            ...
            let items = match &attrs_obj.content {
                ...
                _ => {
                    return Err(CivError::InvalidData(
                        "SignedAttributes content not sequence/set".to_string(),
                    ))
                }
            };
                ...
                if let Ok(attr) = item.as_sequence() {
                    if let Ok(oid) = attr[0].as_oid() {
                        if oid.to_string() == message_digest_oid {
                            let values = attr[1].as_set().map_err(|_| {
                                CivError::InvalidData("Attr values not set".to_string())
                            })?;
                            found_digest = Some(
                                values[0]
                                    .as_slice()
                                    .map_err(|_| {
                                        CivError::InvalidData("Digest not sliceable".to_string())
                                    })?
                                    .to_vec(),
                            );
                        }
            ...
            if let Some(expected_digest) = found_digest {
                if actual_hash != expected_digest {
                    return Err(CivError::InvalidData(format!(
                        "SOD Message Digest mismatch: actual={}, expected={}",
                        hex::encode(actual_hash),
        ...
        if let Some(der) = &sod.signer_cert {
            if !der.is_empty() {
                let (_, cert) = X509Certificate::from_der(der)
                    .map_err(|_| CivError::InvalidData("Failed to parse DSC".to_string()))?;
                ...
                if let Some(signed_attrs) = &sod.signed_attributes {
                    ...
                    match oid.as_str() {
                        ...
                        "1.2.840.10045.2.1" => {
                            ...
                            let verifying_key =
                                VerifyingKey::from_sec1_bytes(raw_key).map_err(|e| {
                                    CivError::CryptoError(format!("Invalid EC key: {}", e))
                                })?;

                            let sig = EcdsaSignature::from_der(&sod.signature).map_err(|_| {
                                CivError::InvalidData("Invalid ECDSA signature format".to_string())
                            })?;

                            verifying_key
                                .verify(&verification_data, &sig)
                                .map_err(|e| {
                                    CivError::AuthenticationFailed(format!(
                                        "PA Signature verification failed: {}",
                                        e
                                    ))
                                })?;
                        }
