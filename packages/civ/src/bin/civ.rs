...
#[cfg(not(target_arch = "wasm32"))]
use civ::{
    CardReader, DriversLicenseController, IdentityController, JpkiController, MyKadController,
    MynaMenkyoController, PassportController, PcscReader, ResidenceCardController, ThaiController,
};
#[cfg(not(target_arch = "wasm32"))]
...
#[cfg(not(target_arch = "wasm32"))]
async fn run_unified_id<R: CardReader + 'static>(
    mut reader: R,
    pin: Option<String>,
    mrz: Option<String>,
    verify: bool,
    ...
    let default_pin = if detected_type == "jpki" {
        "1234"
    } else {
        "123456"
    };
    ...
    if json {
        ...
        if let Some(given) = identity.given_names {
            println!("Given:     {}", given);
        }
        if let Some(kana) = identity.full_name_kana {
            println!("Kana:      {}", kana);
        }
        ...
        if let Some(iss) = identity.issuing_authority {
            println!("Issuer:    {}", iss);
        }
        if let Some(exp) = identity.expiration_date {
            println!("Expires:   {}", exp);
        }
        println!(
            "Verified:  {}",
...
#[cfg(not(target_arch = "wasm32"))]
async fn run_jpki<R: CardReader>(
    mut controller: JpkiController<R>,
    command: JpkiCommands,
) -> anyhow::Result<()> {
    match command {
        JpkiCommands::Retries => {
            ...
            let _ = controller
                .get_auth_pin_retries()
                .await
                .map(|c| println!("Auth PIN: {}", c));
            let _ = controller
                .get_sign_pin_retries()
                .await
                .map(|c| println!("Sign PIN: {}", c));
            let _ = controller
                .get_input_support_pin_retries()
                .await
                .map(|c| println!("Input Support: {}", c));
            let _ = controller
                .get_surface_pin_retries()
                .await
                .map(|c| println!("Surface PIN (12-digit): {}", c));
        }
        JpkiCommands::Cert { type_, output } => {
            let data = if type_ == "sign" {
        ...
        JpkiCommands::Sign {
            ...
            let data_bytes = if let Some(d) = data {
                ...
                return Err(anyhow::anyhow!(
                    "Either --data or --input context is required"
                ));
            };

            let sig = if type_ == "sign" {
                controller
                    .compute_digital_signature(&p, &data_bytes)
                    .await?
            } else {
