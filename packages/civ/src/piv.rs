...
impl<R: CardReader> PivController<R> {
    ...
    pub async fn select_piv_ap(&mut self) -> Result<()> {
        let apdu =
            ApduCommand::new(CLA_ISO, INS_SELECT_FILE, 0x04, 0x0C).with_data(&file_ids::DF_PIV);
    ...
    pub async fn verify_pin(&mut self, pin: &str) -> Result<()> {
        ...
        let apdu = ApduCommand::new(
            CLA_ISO,
            0x20,
            0x00,
            KeyReference::PivCardApplicationPin as u8,
        )
        .with_data(&pin_bytes);
    ...
    pub async fn general_authenticate(
        ...
        if payload.len() > 255 {
            return Err(CivError::InvalidData(
                "Payload too large for simple encoder".to_string(),
            ));
        }
        ...
        let apdu = ApduCommand::new(0x00, 0x87, alg as u8, key_ref as u8).with_data(&data);
    ...
    pub async fn authenticate_user(&mut self, pin: &str) -> Result<()> {
        ...
        let alg = if check_oid(oid, &[1, 2, 840, 113549, 1, 1, 1]) {
            ...
            return Err(CivError::InvalidData(
                "Unsupported Certificate Algorithm".to_string(),
            ));
        };
    ...
    pub async fn read_cert(&mut self, key_ref: KeyReference) -> Result<Vec<u8>> {
        let tag = match key_ref {
            ...
            _ => {
                return Err(CivError::InvalidData(
                    "Invalid Key Reference for Certificate".to_string(),
                ))
            }
        };
    ...
    async fn get_data(&mut self, tag_data: &[u8]) -> Result<Vec<u8>> {
        let apdu = ApduCommand::new(CLA_ISO, 0xCB, 0x3F, 0xFF).with_data(tag_data);
...
mod tests {
    ...
    async fn test_authenticate_user_mock() {
        ...
        let mut controller = PivController::new(MockRelayReader {
