...
use p256::{
    ...
    EncodedPoint, ProjectivePoint, PublicKey, Scalar,
};
use rand_core::OsRng;
...
impl PaceP256 {
    ...
    pub fn perform_mapping_and_generate_key(&mut self) -> Result<Vec<u8>> {
        ...
        let _ = decryptor
            .decrypt_padded_mut::<NoPadding>(&mut z_buf)
    ...
    pub fn compute_shared_secret(&mut self, peer_pk_bytes: &[u8]) -> Result<()> {
        let peer_pk =
            PublicKey::from_sec1_bytes(peer_pk_bytes).context("Invalid Peer Public Key")?;
    ...
    pub fn perform_token_exchange(&mut self, peer_token: &[u8]) -> Result<Vec<u8>> {
        let (_k_enc, k_mac) = self
            .session_keys
            .as_ref()
            .ok_or_else(|| anyhow!("No session keys"))?;

        let my_pk = self
            .my_public_key
            .as_ref()
            .ok_or(anyhow!("No My PK"))?
            .to_encoded_point(false)
            .as_bytes()
            .to_vec();
        let peer_pk = self
        ...
        if !peer_token.is_empty() {
            ...
            if peer_token != expected_8 {
                return Err(anyhow!(
                    "Token Verification Failed. Expected: {:?}, Got: {:?}",
                    expected_8,
                    peer_token
                ));
            }
...
pub fn derive_session_keys_sha256(
    shared_secret: &[u8],
    key_len_bytes: usize,
) -> (Vec<u8>, Vec<u8>) {
...
mod tests {
    ...
    fn test_pace_gm_flow() {
        ...
        let s_len = s_buf.len();
        let _ = encryptor
            .encrypt_padded_mut::<NoPadding>(&mut s_buf, s_len)
            .unwrap();
