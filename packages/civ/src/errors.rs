...
mod tests {
    ...
    fn test_error_display() {
        ...
        assert!(
            format!("{}", CivError::AuthenticationFailed("err".to_string())).contains("failed")
        );
        ...
        assert!(
            format!("{}", CivError::SecureMessagingError("err".to_string()))
                .contains("Secure Messaging")
        );
    ...
    fn test_from_sw_extra() {
        assert!(matches!(
            CivError::from_sw(0x63, 0x00),
            CivError::AuthenticationFailed(_)
        ));

        assert!(matches!(
            CivError::from_sw(0x67, 0x00),
            CivError::InvalidData(_)
        ));

        assert!(matches!(CivError::from_sw(0x69, 0x83), CivError::PinLocked));

        assert!(matches!(
            CivError::from_sw(0x69, 0x85),
            CivError::AccessDenied(_)
        ));

        assert!(matches!(
            CivError::from_sw(0x69, 0x88),
            CivError::SecureMessagingError(_)
        ));

        assert!(matches!(
            CivError::from_sw(0x6A, 0x80),
            CivError::InvalidData(_)
        ));

        assert!(matches!(
            CivError::from_sw(0x6A, 0x86),
            CivError::InvalidData(_)
        ));

        assert!(matches!(
            CivError::from_sw(0x6A, 0x88),
            CivError::NotFound(_)
        ));

        assert!(matches!(
            CivError::from_sw(0x6B, 0x00),
            CivError::InvalidData(_)
        ));

        assert!(matches!(
            CivError::from_sw(0xFF, 0xFF),
            CivError::Communication(_)
        ));
    }
