...
use crate::mock::common::{MockBackend, der_wrap, extract_tlv_value};
        ...
        match cmd.ins {
            0x20 => { // VERIFY
                if cmd.p2 == KeyReference::PivCardApplicationPin as u8 {
                    ...
                    while pin_bytes.last() == Some(&0xFF) { pin_bytes.pop(); }
                    let pin_str = String::from_utf8_lossy(&pin_bytes);
                    if pin_str == self.pin { self.pin_verified = true; (vec![], 0x9000) }
                    else { (vec![], 0x63C3) }
                } else { (vec![], 0x6A88) }
            },
            0xCB => { // GET DATA
                if cmd.data.len() >= 5 && cmd.data[0] == 0x5C {
                    ...
                    if let Some(data) = self.files.get(tag) { (data.clone(), 0x9000) }
                    else { (vec![], 0x6A82) }
                } else { (vec![], 0x6A80) }
            },
            0x87 => { // GENERAL AUTHENTICATE
                if !self.pin_verified { return (vec![], 0x6982); }
                let challenge = extract_tlv_value(&cmd.data, 0x81).unwrap_or_default();
                if challenge.is_empty() { return (vec![], 0x6A80); }
