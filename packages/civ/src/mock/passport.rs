...
use crate::crypto::pace::{
    derive_password_key, derive_session_keys_sha256, PaceMappingType, PaceP256,
};
use crate::crypto::sm::AesSecureMessaging;
use crate::mock::common::{build_tlv, der_wrap, extract_tlv_value, MockBackend, MockSecureSession};
...
impl PassportBackend {
    pub fn new(_password: &str) -> Self {
        ...
        files.insert(
            vec![0x01, 0x0E],
            vec![0x31, 0x10, 0x30, 0x0E, 0x04, 0x0C, 0x01, 0x02, 0x03, 0x04],
        );
    ...
    fn generate_mock_sod(dgs: Vec<(u8, Vec<u8>)>) -> Vec<u8> {
        ...
        for (num, data) in dgs {
            ...
            let item = der_wrap(
                0x30,
                &[der_wrap(0x02, &[num]), der_wrap(0x04, &hash)].concat(),
            );
        ...
        let signed_attrs_inner = [der_wrap(
            0x30,
            &[
        ...
        let signer_info = der_wrap(
            0x30,
            &[
                der_wrap(0x02, &[0x01]),
                der_wrap(
                    0x30,
                    &[der_wrap(0x30, &[]), der_wrap(0x02, &[0x01])].concat(),
                ),
                ...
                if cmd.data.len() >= 2 && cmd.data[0] == 0x7C {
                    ...
                    match inner_tag {
                        0x00 => {
                            ...
                            let encryptor =
                                <Aes128CbcEnc as KeyIvInit>::new(&k_pi.into(), &[0u8; 16].into());
                            let mut nonce_buf = nonce.to_vec();
                            let _ = encryptor
                                .encrypt_padded_mut::<NoPadding>(&mut nonce_buf, 16)
                                .unwrap();
                        ...
                        0x85 => {
                            if let Some(pace) = &mut self.pace_state {
                                let pcd_token =
                                    extract_tlv_value(&cmd.data, 0x85).unwrap_or_default();
                                let server_token = pace.perform_token_exchange(&pcd_token).unwrap();
                                let session = pace.finalize_session().unwrap();
                                self.new_secure_session = Some(MockSecureSession::Pace(
                                    AesSecureMessaging::new(
                                        &session.k_enc,
                                        &session.k_mac,
                                        session.ssc,
                                    )
                        ...
                        0x80 => {
                            let pcd_pk_bytes =
                                extract_tlv_value(&cmd.data, 0x80).unwrap_or_default();
                            if !pcd_pk_bytes.is_empty() {
                                ...
                                self.new_secure_session = Some(MockSecureSession::Pace(
                                    AesSecureMessaging::new(&k_enc, &k_mac, 0).unwrap(),
                                ));
                                (vec![], 0x9000)
                            } else {
                                (vec![], 0x6A80)
